...
void saveScores(const char* filename, const Entry list[], int
count) {
FILE* f = fopen(filename, "w");
if (f == NULL) {
...
// Insert into sorted list (descending). Return 1 if the entrymade
// top-5
int addScore(Entry list[], int* count, const char* name, int
score) {
int pos = *count;
for (int i = 0; i < *count; i++) {
...
int last = (*count < MAX_ENTRIES) ? *count : MAX_ENTRIES -
1;
...
// 3. Keep asking for new scores until the user types "q" as
//the name
char name[NAME_LEN];
int score;
while (1) {
...
// 4. Add if top-5, then save and show the updated
//leaderboard
